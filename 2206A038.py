...
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score, accuracy_score, confusion_matrix, log_loss, hinge_loss
...
class MLCourseGUI(QMainWindow):
    ...
    def handle_missing_data(self, method):
        """Handle missing data using the selected method"""
    ...
    def create_classical_ml_tab(self):
        ...
        svm_group = self.create_algorithm_group(
            ...
            {"C": "double", "Task Type": ["Classification", "Regression"],
    ...
    def train_model(self, model_name, param_widgets):
        """Train the selected classical machine learning model"""
        try:
            ...
            if model_name == "Linear Regression":
                normalize = params.pop("normalize", False)  # Remove 'normalize' param
                self.current_model = LinearRegression(**params)

                if normalize:
                    print("🔹 Applying Standard Scaling to normalize data...")
                ...
                self.current_model = LogisticRegression(C=C, solver=solver, max_iter=1000)

            elif model_name == "Support Vector Machine":
                kernel = params.pop("kernel", "rbf")
                C = params.pop("C", 1.0)
                task_type = params.pop("Task Type")

                if task_type == "Classification":
                    self.current_model = SVC(kernel=kernel, C=C)
                else:
                    epsilon = params.pop("epsilon", 0.1)
                    self.current_model = SVR(kernel=kernel, C=C, epsilon=epsilon)

            elif model_name == "Naive Bayes":
                var_smoothing = params.pop("var_smoothing", 1e-9)
                priors = params.pop("priors", None)
                if priors:
                    priors = [float(p) for p in priors.split(",")]
    ...
    def update_metrics(self, y_pred):
        ...
        if len(np.unique(self.y_test)) > 10:  # More than 10 unique values → Regression
            ...
            y_pred = np.round(y_pred).astype(int)  # Convert regression output to class labels if needed
